/*
  ...
  Render index page
  Latest modified 2016-02-02 16:28
*/
...
var cookieOfUserName = 'JideUploader';
var cookieOfUserLevel = 'JideUploaderLevel';
            ...
            if (oldPassword == thisUser.password) { // Old one matches, password can be changed:
                ...
                fs.writeFile(userFile, userToString, function(error, d) {
                    if (error) {
                        ...
                        res.send({
                            code: 1,
                            msg: 'You have changed your password successfully!<br>Please login again!'
                        });
                    }
                });
                ...
                res.send({
                    code: 0,
                    msg: 'Make sure you have the correct OLD password!'
                });
            }
        }
    }); // end read File
...
router.get('/recentfiles', function(req, res) {
    ...
    fs.readFile(recentFileName, 'utf-8', function(err, data) {
        if (data) {
            ...
            if (showHowManyRecentFiles > len) {
                showHowManyRecentFiles = len;
            }
            result.code = 1;
...
router.get('/clearRecentFiles', function(req, res) {
    fs.writeFile(recentFileName, '', function(err, data) {
        if (err) {
            ...
            res.sendStatus({
                code: 1,
                data: 'Recent files have been cleared successfully!'
            });
        }
    });
});
...
router.post(jdConf.localSaveApi, multipartMiddleware, function(req, res) {
    ...
    fs.rename(tempPath, targetPath, function(err, data) {
        if (err) {
            ...
            // Do Qiniu&AWS upload here:
            var qiniu_uptoken = generateUptoken.Uptoken(qnConf.QiniuJideConfig.Bucket_Name);
            var extra = null;
            var AWSInfo = uploadInfos.AWSInfo;
            // var fileBuffer = new Buffer(1);
            // fileBuffer.write(file);
            var params = {
                Bucket: AWSInfo.bucket,
                Key: uploadDirName + '/' + filenameForCloud,
                ContentType: fileType,
                Body: null,
                ACL: AWSInfo.acl,
                ServerSideEncryption: AWSInfo.ServerSideEncryption
            };
            var s3 = new aws.S3();
            s3.config.region = AWSInfo.region;
            s3.config.update({
                ...
                if (error) {
                    ...
                    qiniu.io.put(qiniu_uptoken, uploadDirName + '/' + filenameForCloud, data, extra, function(err, ret) {
                        if (err) {
                            ...
                            fs.writeFile(recentFileName, text, {
                                flag: 'a'
                            }, function(err, data) {}); // flag 'a' means 'Adding' or 'Appending'
